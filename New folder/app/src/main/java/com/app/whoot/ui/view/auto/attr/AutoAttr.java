package com.app.whoot.ui.view.auto.attr;

import android.view.View;

import com.app.whoot.ui.view.auto.utils.AutoUtil;
 ...
 */
    ...
    public void apply(View view) {
        ...
        if (log) {
            LogUtil.d(" pxVal = " + pxVal + " ," + this.getClass().getSimpleName());
        }
        int val;
        if (useDefault()) {
            val = defaultBaseWidth() ? getPercentWidthSize() : getPercentHeightSize();
            if (log) {
                LogUtil.d(" useDefault val= " + val);
            }
        } else if (baseWidth()) {
            val = getPercentWidthSize();
            if (log) {
                LogUtil.d(" baseWidth val= " + val);
            }
        } else {
            val = getPercentHeightSize();
            if (log) {
                LogUtil.d(" baseHeight val= " + val);
            }
        }
