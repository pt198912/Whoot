package com.app.whoot.base.activity.input;
...
import com.app.whoot.base.impl.FragmentActivityImpl;
//  Created by ruibing.han on 2018/3/27.

//无MVP的FragmentActivity基类 -- 当页面有EditText时 实现点击EditText外面后先进行隐藏软键盘效果
public abstract class BaseInputFragmentActivity extends BaseInputActivity implements FragmentActivityImpl {
    ...
    public void replaceFragment(@NonNull String replaceFragTag, boolean isAddBackStack, boolean isShowAnim, boolean isBackLeft) {
        ...
        if (isShowAnim) {
            if (isBackLeft) {
                //如果往左进栈 --- 左进右出
                transaction.setCustomAnimations(R.anim.in_left, R.anim.out_right, R.anim.in_right, R.anim.out_left);
            } else {
                //如果往右进栈 --- 右进左出
     ...
     */
    public void addFragment(@NonNull String curFragTag, @NonNull String nextFragTag, boolean isShowAnim, boolean isShowLeft) {
        //不是第一次添加
        FragmentManager manager = getSupportFragmentManager();
        ...
        if (isShowAnim) {
            if (isShowLeft) {
                //如果往左切换 --- 左进右出
                transaction.setCustomAnimations(R.anim.in_left, R.anim.out_right, 0, 0);
            } else {
                //如果往右切换 --- 右进左出
                transaction.setCustomAnimations(R.anim.in_right, R.anim.out_left, 0, 0);
            }
        }
